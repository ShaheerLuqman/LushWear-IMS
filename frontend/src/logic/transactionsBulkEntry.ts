...
function splitFromTo(ledgers: Ledger[], text: string): SplitResult {
  ...
  for (const split of splits) {
    ...
    if (fromName && toName && findLedgerByName(ledgers, fromName) && findLedgerByName(ledgers, toName)) {
  ...
  if (findLedgerByName(ledgers, whole)) return { fromName: whole, toName: null };

  const last = splits[splits.length - 1];
  return { fromName: afterFrom.slice(0, last.index).trim(), toName: afterFrom.slice(last.index + last.length).trim() };
}

export interface ParsedBulkEntry { amount: number; description: string; from_account_id: string | null; to_account_id: string | null }
export interface ParsedBulkLine {
  ok: boolean; errors: string[]; lineNo: number; raw: string; blank?: boolean;
  cleaned?: string; amount?: number; particulars?: string; entries?: ParsedBulkEntry[];
}
...
export function parseBulkEntryLine(ledgers: Ledger[], raw: string, lineNo: number): ParsedBulkLine {
  ...
  const named = {
    from: sides.fromName ? findLedgerByName(ledgers, sides.fromName) : null,
    to: sides.toName ? findLedgerByName(ledgers, sides.toName) : null,
  };
  ...
  const from = named.from && named.from.id !== cashId ? named.from : null;
  const to = named.to && named.to.id !== cashId ? named.to : null;
  if (!from && !to) {
  ...
  result.entries = [{
    ...
    description: particulars || defaultTransactionParticulars(from?.name || '', to?.name || ''),
    from_account_id: from ? from.id : null,
  ...
  return parsed.lines.map((p) => {
    ...
    if (p.ok) {
      const summary = (p.entries || []).map((e) => {
        ...
        return `${formatBulkAmount(e.amount)} ${escapeHtml(from)} <i class="fa-solid fa-arrow-right"></i> ${escapeHtml(to)}`;
      }).join(' , ');
