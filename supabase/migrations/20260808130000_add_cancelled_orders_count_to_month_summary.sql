-- Adds cancelled_orders_count to get_month_summary_totals so the Month Summary
-- view can show how many of the period's orders were excluded as cancelled,
-- alongside the already-excluded total_orders/net_sales/etc figures.
-- period_orders already filters cancelled out for every other metric here, so
-- the new count is taken straight from shopify_orders rather than from that CTE.
