...
BEGIN
    ...
    -- code is unique per org too (idx_ledgers_org_code), and an org may already keep
    -- a hand-made ledger on the number a system account wants. Walk to the next free
    -- number rather than aborting the seeding run; a non-numeric clash gives up to a
    -- NULL code, which the partial index allows any number of.
...
-- voucher against the courier's ledger. Per-order cash postings would never
-- reconcile against a bank statement.
--
-- folio is normalised (the "-API" suffix marking a tracking-derived folio is
-- stripped) so one real CPR settled partly from a CSV and partly from the API is
...
-- Cutover for order posting: orders dispatched before this post nothing, so a
-- pre-cutover order settling afterwards cannot credit a courier receivable that
-- was never debited. Per-org because posting functions are multi-tenant; not
-- exposed in the UI, since moving it later would rewrite closed months.
...
-- Product Analytics page: per-product units & revenue over a date range plus the
-- equal-length window right before it, a size (variant) breakdown, distinct order
-- counts per collection, and a bucketed sales trend - all from
-- shopify_orders.line_items in one call. Basis: non-cancelled, non-replacement
-- orders, filtered on (order_receiving_date AT TIME ZONE 'UTC')::date. A line is
-- attributed to a product by variant_id then product_id; unresolved lines are
-- grouped by lowercased name and returned with product_id = null for the route to
-- name-match against the catalog. See
-- supabase/migrations/20260903010000_product_analytics_rpc.sql.
